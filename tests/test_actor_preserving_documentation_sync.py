"""Keep the actor-preserving PPO theory-first preregistration synchronized with frozen executable contracts."""
...
DOCUMENT = ROOT / "docs/actor_preserving_ppo.md"
...
PPO_CONFIG = ATTITUDE_ENV.parent / "agents/rl_games_actor_preserving_ppo_cfg.yaml"
CHECKPOINT_SELECTION_VALIDATION = ROOT / "benchmarks/checkpoint_selection/validation_results.csv"
CHECKPOINT_SELECTION_FORMAL = ROOT / "benchmarks/checkpoint_selection/formal_results.csv"
...
def _preregistered() -> dict[str, Any]:
    ...
    assert len(matches) == 1, "actor-preserving PPO document must contain exactly one machine-readable parameter block"
    payload = yaml.safe_load(matches[0])
    assert set(payload) == {"actor_preserving_preregistered_config"}
    return payload["actor_preserving_preregistered_config"]


def test_actor_preserving_preregistered_parameters_match_frozen_task_and_ppo() -> None:
    ...
    assert ppo["algo"]["name"] == "actor_preserving_ppo"
    ...
    actor_preserving_config = config["actor_preserving"]
    assert actor_preserving_config["schema_version"] == prereg["migration"]["schema_version"]
    assert actor_preserving_config["warmup_epochs"] == prereg["warmup_epochs"]
    assert (
        actor_preserving_config["freeze_lr_scheduler_during_warmup"]
        == prereg["freeze_lr_scheduler_during_warmup"]
    )
    assert actor_preserving_config["freeze_observation_rms"] == prereg["freeze_observation_rms"]
    assert actor_preserving_config["bc_anchor_type"] == prereg["bc_anchor"]["type"]
    assert actor_preserving_config["bc_anchor_coefficient"] == pytest.approx(prereg["bc_anchor"]["coefficient"])
...
def test_actor_preserving_seed_roles_and_evaluation_match_frozen_checkpoint_selection_protocol() -> None:
    prereg = _preregistered()
    with CHECKPOINT_SELECTION_VALIDATION.open(newline="", encoding="utf-8") as stream:
        validation_rows = list(csv.DictReader(stream))
    with CHECKPOINT_SELECTION_FORMAL.open(newline="", encoding="utf-8") as stream:
...
def test_actor_preserving_theory_traceability_targets_exist() -> None:
    prereg = _preregistered()
    assert prereg["migration"]["schema_version"] == "actor-preserving-separate-v1"
    assert prereg["migration"]["critic_seed"] == 2026
    assert prereg["migration"]["parity_max_abs_error"] == pytest.approx(1.0e-5)
    for path in (
        ROOT / "docs/heave_precision_theory.md",
        ROOT / "docs/physical_deck_theory.md",
        ROOT / "docs/physical_deck_attitude_theory.md",
        ROOT / "docs/imitation_hybrid_paper.md",
        ROOT / "docs/checkpoint_selection_and_policy_drift.md",
        DOCUMENT,
