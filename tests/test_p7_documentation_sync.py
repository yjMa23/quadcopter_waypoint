"""Keep the P7 paper-style theory document synchronized with executable contracts."""
...
PAPER = ROOT / "docs/p7_imitation_hybrid_paper.md"
...
def _sync_values() -> dict[str, str]:
    ...
    assert match, "P7 theory document is missing the CODE_SYNC block"
    values: dict[str, str] = {}
...
def test_p7_paper_code_sync_contract() -> None:
...
def test_p7_paper_traceability_paths_exist() -> None:
    for path in (
        DATASET_SOURCE,
        POLICY_SOURCE,
        BASE_ENV_SOURCE,
        ...
        ROOT / "benchmarks/phase7_imitation_hybrid/summary.json",
        ROOT / "benchmarks/phase7_imitation_hybrid/commands.txt",
    ):
