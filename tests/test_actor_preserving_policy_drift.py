"""Extended actor-preserving PPO policy-drift metrics and hash semantics."""
...
def test_zero_drift_is_exact_for_identical_state() -> None:
    state = _state()
    metrics = compute_actor_preserving_drift_metrics(state, copy.deepcopy(state), torch.randn(128, 22))
...
def test_actor_critic_rms_and_sigma_drift_are_distinguished() -> None:
    ...
    metrics = compute_actor_preserving_drift_metrics(reference, candidate, torch.randn(128, 22))
...
def test_drift_rejects_invalid_observation_batch() -> None:
    ...
    with pytest.raises(ValueError):
        compute_actor_preserving_drift_metrics(state, state, torch.randn(8, 21))
    invalid = torch.randn(8, 22)
    invalid[0, 0] = float("inf")
    with pytest.raises(FloatingPointError):
        compute_actor_preserving_drift_metrics(state, state, invalid)
