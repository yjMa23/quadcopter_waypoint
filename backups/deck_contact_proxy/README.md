# Deck-Contact Proxy Baseline Historical Archive

> Historical record only. This file preserves the state known at the Deck-Contact Proxy Baseline freeze and is not the current project plan.
...
```text
Deck-Contact Proxy Baseline: moving landing pad with deck-contact proxy success condition
```
...
This version was fine-tuned from the Moving-Pad Precision Baseline stable checkpoint:

```text
logs/rl_games/quadcopter_ship_landing/2026-06-29_11-16-09/nn/last_quadcopter_ship_landing_ep_600_rew_41.586376.pth
```

Training command used for Deck-Contact Proxy Baseline v1:
...
Deck-Contact Proxy Baseline upgrades the task from a simple geometric height threshold to a deck-contact proxy. This is a better baseline before adding deck heave, roll, and pitch.
...
```text
Heave-Precision Proxy: add deck heave / vertical sinusoidal motion while keeping roll and pitch fixed.
```

Actual later progression:

```text
Heave-Precision Proxy: Heave Precision marker/contact-proxy baseline, completed and frozen.
Physical Deck: horizontal PhysicalDeck with rigid/kinematic deck and contact sensing, completed.
