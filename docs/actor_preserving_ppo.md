# actor-preserving PPO：Actor-Preserving PPO（保守策略微调）

> 文档状态：**设计预注册、pilot、正式训练、validation 选模、独立 formal test、drift、targeted headless 视频和可复现实验包均已完成**。初始设计在任何 actor-preserving PPO 核心训练代码、pilot 或正式实验之前写入；实验后只追加真实结果，不删除或改写原预测。

## 1. 问题定义与冻结边界

imitation-learning benchmark 的 BC actor 在 physical-deck-attitude task 实体倾斜运动甲板上已具有较高稳定降落率，但普通在线 PPO 会明显破坏策略；checkpoint-selection analysis 进一步发现三个训练 seed 的任务指标最优 checkpoint 都出现在最早保存的 epoch 10，而 reward-selected checkpoint 只有 78.08% settled landing。checkpoint-selected BC+PPO 达到 91.67%，仍未稳定达到 92%。

actor-preserving PPO 的问题定义为：在固定 physical-deck-attitude task 任务语义、既有 BC 初始化和每 seed 200 epochs/1,228,800 environment steps 预算下，减少 early actor drift，使 PPO 改善或至少不快速破坏 BC 策略。

以下内容冻结：任务 ID、动力学、甲板模型、22 维 observation、4 维 action、动作缩放、reward、termination、接触/稳定/失败判定、训练预算和正式评估指标。actor-preserving PPO 不修改 reward，不放宽 success 或 safety，不更换 expert dataset，不减少正式 episodes，不隐藏失败 seed。当前策略是 state-based，不包含相机图像。

## 2. 历史证据

imitation-learning benchmark 三 seed、每 seed 256 episodes：teacher 94.66% ±0.49%，PPO scratch 0.91% ±0.66%，BC-only 88.28% ±0.55%，普通 BC+PPO 76.69% ±9.16%，learning-rate=1e-5 诊断 63.67% ±18.02%。

checkpoint-selection analysis 使用 validation seeds 145/146/147 选模、formal test seeds 245/246/247 独立测试。train seeds 42/43/44 均选择 epoch10；metric-selected 91.67% ±3.03%，reward-selected 78.08% ±8.66%。epoch10 action MSE 相对 BC 约为 `6.1e-4`、`6.5e-4`、`9.2e-4`。这些是统计证据，不足以单独证明 actor drift 是唯一因果机制。

## 3. 候选方案比较

| 候选 | 优点 | 当前不单独采用的原因 |
|---|---|---|
| A 仅降低全局 learning rate | 实现简单 | imitation-learning benchmark 的 1e-5 诊断更差且方差更大；不能隔离 critic 梯度和 RMS drift |
| B 仅按 validation settled landing 早停 | 避免后期退化 | checkpoint-selection analysis 已采用，仍未稳定达到 92%；不能阻止 epoch10 前漂移 |
...
| F BC action L2 anchor | 与 checkpoint-selection analysis action MSE 对齐，梯度直接 | coefficient 需受控 pilot，不保证闭环必然改善 |
...
| I DAgger/recovery demonstrations | 可改善 covariate shift | 需要重新采集和标注，超出当前研究范围 |
| J residual policy | 强保护 BC baseline | 改变策略参数化和比较语义，工程复杂度更高 |

本文采用：**separate actor/critic + 10 epochs critic-only warm-up + frozen observation RMS + on-policy BC mean-action L2 anchor + validation settled landing 选模**。它直接针对 imitation-learning benchmark/checkpoint-selection analysis 的共享梯度、cold critic、RMS 变化和 early action drift，不改变环境、不需要新 demonstration，且可通过 hash、parity、gradient isolation 和闭环评估证伪。
...
imitation-learning benchmark 配置 `network.separate: false`，RL-Games 的 Adam optimizer 包含 model 全部参数，真实总损失为
...
输入为 imitation-learning benchmark `bc_init_rlgames.pth` 和 actor-preserving PPO separate template/schema。迁移规则：
...
\[
\epsilon_{parity}=\max|\operatorname{clip}(\mu_{BC},-1,1)-\operatorname{clip}(\mu_{actor-preserving PPO},-1,1)|\le10^{-5}.
\]
...
actor-preserving PPO 全训练和 resume 后保持
...
因此 L2 与 KL 有明确关系；L2 更直接对应 checkpoint-selection analysis action MSE，且 \(\lambda=0\) 可验证退化为无 anchor 控制组。

imitation-learning benchmark 首轮 actor loss 约 0.03，epoch10 action MSE 约 `6e-4`–`9e-4`。预注册 pilot 系数为 0、10、50：系数 10 对应约 0.006–0.009 的约束量级，系数 50 对应约 0.03–0.045，与早期 actor loss 同量级。禁止再增加候选或按 seed 使用不同系数。

reference actor state_dict 写入每个 actor-preserving PPO checkpoint，同时记录源 BC SHA256；resume 时加载并校验，不依赖可被移动的外部路径。

## 9. Checkpoint Selection 与 Policy Drift

每个正式训练 seed 保存 epoch0、epoch10 warm-up 边界、每 10 epochs、last 和 reward-selected checkpoint。validation 前先按 actor+RMS SHA 去重，但保留 checkpoint SHA。选择规则沿用 checkpoint-selection analysis 真实代码：最大 settled landing、最小 deck miss、最小 hard contact、最小 touchdown distance、最早 epoch。

固定 drift observation batch 沿用 checkpoint-selection analysis dataset test split，报告 checkpoint/actor/critic SHA、obs RMS hashes/count、action MSE/max error、actor/critic relative L2、RMS drift、sigma drift。action drift 与 settled landing 只作统计关联，不作因果结论。
...
正式比较至少包含：BC-only、checkpoint-selected BC+PPO、actor-preserving PPO metric-selected、actor-preserving PPO reward-selected、actor-preserving PPO last。安全目标沿用 physical-deck-attitude task：ground crash ≤1%、hard contact ≤2%、timeout ≤3%；性能参考目标 settled landing ≥92%。未达到时保留负结果并完成诊断。
...
该块是 actor-preserving PPO 参数唯一文档源。pilot 已选择系数 50，并同步更新正式 YAML、文档同步测试和 `benchmarks/actor_preserving_ppo/preregistered_config.yaml`；不得维护第二个冲突参数块。
...
输入为 imitation-learning benchmark BC-init checkpoint、dataset manifest 与 actor-preserving PPO config；输出为不覆盖旧文件的 separate checkpoint。验证 shape/key、来源 SHA、actor/RMS parity、actor/critic storage isolation、optimizer/epoch reset。
...
输入 validation CSV/manifest，输出每 train seed 唯一 checkpoint；复用 checkpoint-selection analysis tie-break，formal seeds 不进入输入。
...
|---|---|---|---|---|
| separate migration | 预定 `imitation/actor_preserving_checkpoint.py::build_actor_preserving_separate_checkpoint` | `migration.*`, `network.*` | `test_actor_preserving_checkpoint_migration.py` | `checkpoint_hashes.json` |
| gradient isolation | RL-Games separate network + 预定 helper | `network.separate` | `test_actor_preserving_actor_critic_separation.py` | warm-up logs/hashes |
| warm-up | 预定 `imitation/actor_preserving_agent.py::ActorPreservingA2CAgent` | `warmup_epochs` | `test_actor_preserving_critic_warmup.py` | training metadata |
| frozen RMS | 同 agent `_enforce_frozen_obs_rms` | `freeze_observation_rms` | `test_actor_preserving_frozen_obs_rms.py` | RMS hashes |
| BC anchor | 同 agent `_bc_anchor_loss` | `bc_anchor.*` | `test_actor_preserving_bc_anchor.py` | TensorBoard/policy drift |
| checkpoint selection | `imitation/checkpoint_sweep.py::select_validation_best` | seeds/evaluation | `test_actor_preserving_checkpoint_selection.py` | validation CSV/selection JSON |
| drift | 预定 `imitation/actor_preserving_drift.py` | fixed batch/schema | `test_actor_preserving_policy_drift.py` | `policy_drift.csv/json` |
| 文档同步 | 本文参数块 + actor-preserving PPO YAML/defaults | 全部预注册键 | `test_actor_preserving_documentation_sync.py` | pytest report |
...
因此先修改理论再修改代码：新增 `freeze_lr_scheduler_during_warmup: true`，epoch1–10 的 optimizer LR 固定为基础 `1e-4`，epoch11 从基础值恢复 adaptive scheduler。该修正对所有候选和 seed 一致，不修改环境、训练预算、anchor 候选、validation 或 formal test 协议。首个 smoke 作为失败诊断证据保留在 `logs/rl_games/actor_preserving_smoke/seed942`，不作为正式结果。
...
旧 sweep inventory 的 `actor_sha256` 历史口径同时包含 actor 与 observation RMS；actor-preserving PPO 正式产物额外给出无歧义的 `actor_weights_sha256`、`critic_sha256` 和 `observation_rms_sha256`，不覆盖历史字段。

## 18. 独立 formal test 结果

formal test 仅在 selection 冻结后运行，固定 test seeds 245/246/247、每 checkpoint/seed 256 episodes。actor-preserving PPO/BC 共 8 个去重物理 checkpoint × 3 seeds = 24 条评估、6,144 episodes，全部 `completed`，无 failed/running 条目。frozen teacher 复用相同正式协议下已有的 3 条、768 episodes 原始评估。
...
| ordinary BC+PPO | 768 | 76.6927% | 23.0469% | 0.9115% | 0.0000% | 0.0000% |
| checkpoint-selected BC+PPO | 2304 | 91.6667% | 8.3333% | 0.3038% | 0.0000% | 0.0000% |
| **actor-preserving PPO metric-selected** | **2304** | **96.7448%** | **3.1684%** | **0.0868%** | **0.0000%** | **0.0434%** |
| actor-preserving PPO reward-selected | 2304 | 95.3993% | 4.5139% | 0.1302% | 0.0000% | 0.0434% |
| actor-preserving PPO epoch200 last | 2304 | 92.4045% | 7.5521% | 0.0434% | 0.0000% | 0.0434% |

actor-preserving PPO metric-selected 为 2229/2304 settled landing，Wilson 95% CI `[95.9388%, 97.3952%]`。三个 training seed 的独立 test settled 分别为 97.7865%、96.4844%、95.9635%，均达到 90% 和 92%。聚合结果比 BC 高 10.5469 个百分点，比 checkpoint-selection analysis 高 5.0781 个百分点，比 frozen teacher 高 2.0833 个百分点。

actor-preserving PPO metric-selected 的其他核心指标：contact success 99.4792%，touchdown distance mean 0.05443 m、p95 0.10073 m，first-contact XY error mean 0.05032 m，normal relative speed mean -0.14705 m/s，tangential relative speed mean 0.16119 m/s，body/deck normal angle mean 0.02921 rad，maximum penetration mean 0.01988 m。

必须保留的负面信息：BC formal test 的 hard contact 恰为 0，而 actor-preserving PPO metric-selected 为 2/2304（0.0868%），因此 hard contact 相对 BC 在数值上恶化，虽然仍远低于 2% 安全目标。actor-preserving PPO metric-selected 另有 73 deck miss、1 timeout、0 ground crash；不能声称所有安全维度都严格优于 BC。last checkpoint 低于 validation-selected，说明 anchor 限制 drift 但不保证持续更新单调改善。
...
| 4 | observation RMS drift 为 0 | supported | 所有 actor-preserving PPO checkpoint 的 mean/variance/count drift 均为 0，RMS SHA 保持 `7aa08a...` |
...
```text
benchmarks/actor_preserving_ppo/videos/selected_success.mp4
benchmarks/actor_preserving_ppo/videos/selected_success.npz
benchmarks/actor_preserving_ppo/videos/selected_success.json
benchmarks/actor_preserving_ppo/videos/selected_failure.mp4
benchmarks/actor_preserving_ppo/videos/selected_failure.npz
benchmarks/actor_preserving_ppo/videos/selected_failure.json
benchmarks/actor_preserving_ppo/videos/video_manifest.json
```
...
actor-preserving PPO 在冻结方法和独立 test 下达到 96.74% settled landing，超过 90%、92%、BC、checkpoint-selection analysis 和 frozen teacher，并显著降低 deck miss。它支持 separate actor/critic、critic warm-up、冻结 RMS 和 BC anchor 组合在当前任务中有效保护强 BC actor，但不能单独证明某一个组件是提升的唯一原因，也不能把 policy drift 的相关性解释为严格因果。

不能声称：策略使用视觉图像；结果覆盖 yaw、随机波谱、完整六自由度船舶或真实水动力；所有安全指标均比 BC 更好；epoch200 优于 validation-selected；自动 headless 视频已经完成人工 GUI 验收。完整机器可解析证据位于 `benchmarks/actor_preserving_ppo/`。
