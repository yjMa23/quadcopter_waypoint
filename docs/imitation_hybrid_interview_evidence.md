# imitation-learning benchmark 专家数据、Behavior Cloning 与 BC+PPO 面试证据
...
imitation-learning benchmark 在冻结的 physical-deck-attitude task 实体运动甲板降落任务上比较四种策略：
...
Expert 使用冻结的 physical-deck-attitude task PPO checkpoint：
...
```text
logs/imitation/expert_dataset/manifest.json
...
```text
logs/imitation/behavior_cloning/best_bc.pth
...
- env state 清空；
- fixed sigma 保持 physical-deck-attitude task PPO 配置；
- 写入来源 BC、dataset hash、template checkpoint 和迁移元数据。

生成 checkpoint：

```text
logs/imitation/behavior_cloning/bc_init_rlgames.pth
...
```text
task: same frozen physical-deck-attitude task task
...
```text
benchmarks/imitation_hybrid/rollout_cases/teacher_success.npz
benchmarks/imitation_hybrid/rollout_cases/bc_success.npz
benchmarks/imitation_hybrid/rollout_cases/bc_ppo_success.npz
benchmarks/imitation_hybrid/rollout_cases/scratch_timeout_failure.npz
```
