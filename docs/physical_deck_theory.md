# physical-deck task：实体水平运动甲板接触理论与证据说明

> 状态：历史冻结说明。physical-deck task 首次把成功判定从几何接触代理切换为真实实体甲板与过滤 ContactSensor。

## 1. 设计目标与基线问题

heave-precision task 的 marker/间隙代理无法证明真实碰撞发生，也不能区分安全接触、硬碰撞、滑出甲板和撞地。physical-deck task 保持水平匀速与竖直升沉，新增 kinematic rigid deck、ground slab、摩擦材料、过滤接触传感器和稳定保持语义。任务 ID 为 `Isaac-Quadcopter-ShipLanding-PhysicalDeck-Direct-v0`。

## 2. 状态、观测、动作与坐标系

世界系 \(\mathcal F_W\)、机体系 \(\mathcal F_B\)。甲板姿态恒为单位四元数，因此甲板系轴与世界系平行。策略继续使用 heave-precision task 的 16 维观测和 4 维动作，保持 checkpoint 可迁移：
...
physical-deck task 沿用 heave-precision task 逐步 reward，并加入实体接触落点惩罚：
...
冻结配置的关键权重为：`predicted_pad_error=-8`、`center_precision=-30`、`center_precision_square=-80`、`descent_vel=-6`、`rel_vel=-1`、`near_pad_horizontal_rel_vel=-7`、`landing_bonus=80`、`crash_penalty=-30`。其余基础项与 heave-precision task 同源。
...
PPO ratio、clipped objective 和 GAE 与 heave-precision task 相同。当前同族配置：gamma 0.99、GAE lambda 0.95、clip 0.2、learning rate 1e-4、horizon 24、minibatch 384、mini epochs 5、critic coefficient 2、entropy coefficient 0、bounds coefficient 1e-4、fixed sigma。
...
|---|---|---|---|---|
| 实体甲板、ground slab、ContactSensor | `.../quadrotor_ship_landing_physical_deck_env.py::QuadcopterShipLandingPhysicalDeckEnvCfg/_setup_scene` | `deck`, `ground_slab`, `*_contact_sensor` | task smoke；评估工具测试 | `benchmarks/physical_deck/summary.json` |
| hard/safe/miss/crash | 同文件 `_compute_landing_terms` | `contact_force_threshold`, `safe_*`, `hard_*`, `deck_edge_margin` | actor-preserving PPO 文档冻结语义检查 | physical-deck task 原始 CSV |
...
| 正式聚合 | `scripts/rl_games/eval_metrics.py` | seeds、episodes | `tests/test_eval_metrics_utils.py` | `benchmarks/physical_deck/summary.json` |
