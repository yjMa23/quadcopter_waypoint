"""Select P8B checkpoints and finalize the reproducible formal benchmark."""
...
def _p8b_model_hashes(checkpoint_path: str) -> dict[str, str | None]:
...
def _flatten_entry(entry: Mapping[str, Any]) -> dict[str, Any]:
    ...
    hashes = _p8b_model_hashes(checkpoint_path)
    return {
...
def _combine_inventories(paths: Sequence[str | Path]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for path in paths:
        for record in _read_json(path)["records"]:
            ...
            hashes = _p8b_model_hashes(resolved)
            by_path[resolved] = {
                **record,
                "path": resolved,
...
def _training_run_summary(run_dir: Path, seed: int) -> dict[str, Any]:
    ...
    reward_selected = run_dir / "nn" / "p8b_formal_lambda50.pth"
    return {
        "training_seed": seed,
...
def _finalize_mode(args: argparse.Namespace) -> None:
    ...
    if {str(Path(row["checkpoint_path"]).resolve()) for row in formal_physical} != allowed:
        raise ValueError("formal manifest does not contain exactly the preregistered P8B checkpoint plan")
    ...
    p7 = _read_json(args.p7_summary)
    p8a = _read_json(args.p8a_summary)
    comparison_rows = [
        {"method": "Frozen PPO teacher", **role_summaries["frozen_teacher"]},
        {"method": "BC epoch 0", **role_summaries["bc_only"]},
        _historical_row("P7 ordinary BC+PPO", p7["formal_evaluation"]["BC+PPO"]["aggregate"]),
        {"method": "P8A metric-selected", **p8a["formal_comparison"]["metric-selected BC+PPO"]},
        {"method": "P8B metric-selected", **role_summaries["metric_selected"]},
        {"method": "P8B reward-selected", **role_summaries["reward_selected"]},
        {"method": "P8B epoch-200 last", **role_summaries["last"]},
    ]
    ...
    prediction_verification = {
        "predictions": [
            ...
            },
            {
                ...
                "limitations": "Measured on the frozen P7 test observation split.",
            },
            {
                "id": 3,
            ...
            },
            {
                ...
                "interpretation": "The frozen observation normalization contract held for all inventoried P8B checkpoints.",
    ...
    p8a_rate = float(p8a["formal_comparison"]["metric-selected BC+PPO"]["settled_landing_rate"])
    summary = {
        "benchmark": "P8B actor-preserving PPO",
        ...
        "answers": {
            "p8b_metric_selected_exceeds_bc": metric_rate > bc_rate,
            "p8b_metric_selected_exceeds_p8a_91_67_percent": metric_rate > p8a_rate,
            "p8b_metric_selected_reaches_90_percent": metric_rate >= 0.90,
            "p8b_metric_selected_reaches_92_percent": metric_rate >= 0.92,
    ...
    readme = f"""# P8B Actor-Preserving PPO 正式 Benchmark
...
| P8A metric-selected | {p8a_rate:.2%} | {float(p8a['formal_comparison']['metric-selected BC+PPO']['deck_miss_rate']):.2%} | {float(p8a['formal_comparison']['metric-selected BC+PPO']['hard_contact_rate']):.2%} |
| **P8B metric-selected** | **{metric_rate:.2%}** | **{role_summaries['metric_selected']['deck_miss_rate']:.2%}** | **{role_summaries['metric_selected']['hard_contact_rate']:.2%}** |
| P8B reward-selected | {reward_rate:.2%} | {role_summaries['reward_selected']['deck_miss_rate']:.2%} | {role_summaries['reward_selected']['hard_contact_rate']:.2%} |
| P8B epoch200 last | {last_rate:.2%} | {role_summaries['last']['deck_miss_rate']:.2%} | {role_summaries['last']['hard_contact_rate']:.2%} |

P8B metric-selected 为 {role_summaries['metric_selected']['settled_episodes']}/{role_summaries['metric_selected']['episodes']}，Wilson 95% CI 为 [{role_summaries['metric_selected']['settled_landing_rate_95ci_wilson_low']:.2%}, {role_summaries['metric_selected']['settled_landing_rate_95ci_wilson_high']:.2%}]。达到 90%：`{metric_rate >= 0.90}`；达到 92%：`{metric_rate >= 0.92}`。
...
def main() -> None:
    ...
    select.add_argument("--output_dir", default="benchmarks/phase8b_actor_preserving_ppo")
    ...
    finalize.add_argument("--p7_summary", required=True)
    finalize.add_argument("--p8a_summary", required=True)
    finalize.add_argument("--video_manifest")
    finalize.add_argument("--output_dir", default="benchmarks/phase8b_actor_preserving_ppo")
