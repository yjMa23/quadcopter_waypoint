"""Analyze actor-preserving PPO actor, critic, action, RMS, and fixed-sigma drift on one frozen observation split."""
...
from quadcopter_waypoint.imitation.actor_preserving_drift import compute_actor_preserving_drift_metrics
...
def main() -> None:
    ...
    if len(reference_records) != 1:
        raise ValueError("actor-preserving PPO drift analysis requires exactly one canonical BC initialization")
    ...
    for record in canonical:
        ...
        metrics = compute_actor_preserving_drift_metrics(reference_model, model, observations)
        row = {
        ...
        rows.append(row)
        print(
            f"[ACTOR_PRESERVING_DRIFT] seed={record.train_seed} kind={record.kind} epoch={record.epoch} "
