"""Build reproducible P8B pilot/formal benchmark tables from raw manifests and drift JSON."""
...
def _coefficient(path: str) -> float | None:
    match = re.search(r"p8b_(?:pilot|formal)_lambda(?P<value>\d+(?:p\d+)?)", path)
    if match:
...
def _write_preregistered_config(output: Path, selected_coefficient: float) -> None:
    payload = {
        "p8b_preregistered_config": {
            ...
            "migration": {"schema_version": "p8b-separate-v1", "critic_seed": 2026, "parity_max_abs_error": 1.0e-5},
            "warmup_epochs": 10,
...
def main() -> None:
    ...
    parser.add_argument("--output", default="benchmarks/phase8b_actor_preserving_ppo")
