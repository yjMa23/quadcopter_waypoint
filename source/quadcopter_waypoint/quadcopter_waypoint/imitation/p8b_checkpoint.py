"""P8B shared-to-separate checkpoint migration and deterministic parity helpers."""
...
def load_p8b_params(config_path: str | Path) -> dict[str, Any]:
    payload = yaml.safe_load(Path(config_path).read_text(encoding="utf-8"))
    if not isinstance(payload, Mapping) or "params" not in payload:
        raise ValueError("P8B config must contain a params mapping")
    params = copy.deepcopy(payload["params"])
    if params["network"].get("separate") is not True:
        raise ValueError("P8B network.separate must be true")
    if params["algo"].get("name") != "p8b_actor_preserving":
        raise ValueError("P8B algo.name must be p8b_actor_preserving")
    return params


def build_p8b_model(params: Mapping[str, Any], seed: int = 2026) -> nn.Module:
    """Build the exact pure-Python RL-Games model used by P8B."""
...
def build_p8b_separate_checkpoint(
    ...
    """Create a fresh P8B checkpoint while preserving the BC actor and observation RMS exactly."""
    source_path = Path(source_checkpoint).expanduser().resolve()
    config = Path(config_path).expanduser().resolve()
    output = Path(output_path).expanduser().resolve()
    manifest = Path(dataset_manifest).expanduser().resolve()
    if output.exists():
        raise FileExistsError(f"refusing to overwrite existing P8B checkpoint: {output}")
    if not source_path.is_file() or not manifest.is_file() or not config.is_file():
        raise FileNotFoundError("source checkpoint, dataset manifest, and P8B config must exist")

    source_payload = torch.load(source_path, map_location="cpu", weights_only=False)
    if "model" not in source_payload or "p7_bc_initialization" not in source_payload:
        raise ValueError("source must be the fresh P7 BC-initialized RL-Games checkpoint")
    source_model = source_payload["model"]
    params = load_p8b_params(config)
    model = build_p8b_model(params, seed=critic_seed)
    ...
    metadata = {
        ...
        "source_bc_checkpoint": source_payload["p7_bc_initialization"].get("source_bc_checkpoint"),
        "source_bc_checkpoint_sha256": source_payload["p7_bc_initialization"].get("source_bc_checkpoint_sha256"),
        ...
        "observation_normalization": "copied_from_p7_bc_init_and_frozen",
        "fixed_sigma": "copied_from_p7_bc_init",
        "warmup_epochs": int(params["config"]["p8b"]["warmup_epochs"]),
        "freeze_lr_scheduler_during_warmup": bool(
            params["config"]["p8b"]["freeze_lr_scheduler_during_warmup"]
        ),
        "base_learning_rate": learning_rate,
        "freeze_observation_rms": bool(params["config"]["p8b"]["freeze_observation_rms"]),
        "bc_anchor_type": params["config"]["p8b"]["bc_anchor_type"],
        "bc_anchor_coefficient": float(params["config"]["p8b"]["bc_anchor_coefficient"]),
...
def checkpoint_parity_error(
    source_checkpoint: str | Path, p8b_checkpoint: str | Path, raw_observation: torch.Tensor
) -> float:
    source = torch.load(Path(source_checkpoint), map_location="cpu", weights_only=False)
    target = torch.load(Path(p8b_checkpoint), map_location="cpu", weights_only=False)
