"""Project-local RL-Games actor-preserving PPO agent for P8B."""
...
class ActorPreservingA2CAgent(a2c_continuous.A2CAgent):
    ...
    def __init__(self, base_name: str, params: dict[str, Any]):
        p8b_config = copy.deepcopy(params["config"].get("p8b", {}))
        if p8b_config.get("schema_version") != SCHEMA_VERSION:
            raise ValueError(f"config p8b.schema_version must be {SCHEMA_VERSION}")
        super().__init__(base_name, params)
        if not self.model.a2c_network.is_separate_critic():
            raise ValueError("P8B requires network.separate=true")

        self.p8b_config = p8b_config
        self.warmup_epochs = int(p8b_config["warmup_epochs"])
        ...
        self.freeze_observation_rms = bool(p8b_config["freeze_observation_rms"])
        self.bc_anchor_type = str(p8b_config["bc_anchor_type"])
        self.bc_anchor_coefficient = float(p8b_config["bc_anchor_coefficient"])
        if self.bc_anchor_type != "mse_mean_action" or self.bc_anchor_coefficient < 0.0:
            raise ValueError("P8B supports only a non-negative mse_mean_action anchor")
    ...
    def train_epoch(self):
        ...
        self.schedule_type = "p8b_frozen_warmup"
        self.last_lr = self.base_learning_rate
        self.update_lr(self.base_learning_rate)
        try:
            return super().train_epoch()
    ...
    def calc_gradients(self, input_dict: dict[str, torch.Tensor]) -> None:
        ...
        with torch.cuda.amp.autocast(enabled=self.mixed_precision):
            ...
            if not torch.isfinite(loss):
                raise FloatingPointError("P8B total loss is NaN or Inf")
            if self.multi_gpu:
    ...
    def set_full_state_weights(self, weights: dict[str, Any], set_epoch: bool = True) -> None:
        ...
        if not isinstance(reference_state, dict):
            raise ValueError("P8B checkpoint is missing embedded reference_actor_state")
    ...
    def write_stats(
        ...
        for key in (
            ...
            if not math.isfinite(value):
                raise FloatingPointError(f"P8B diagnostic {key} is NaN or Inf")
            self.writer.add_scalar(f"p8b/{key}", value, frame)
        self.writer.add_scalar("p8b/warmup_active", float(diagnostics["warmup_active"]), frame)
        self.writer.add_scalar("p8b/actor_trainable", float(diagnostics["actor_trainable"]), frame)
        print("[P8B_DIAGNOSTIC] " + json.dumps(diagnostics, sort_keys=True))


def register_p8b_runner(runner: Any) -> None:
    """Register the project-local agent/player without modifying installed RL-Games source."""
    runner.algo_factory.register_builder(
        "p8b_actor_preserving", lambda **kwargs: ActorPreservingA2CAgent(**kwargs)
    )
    runner.player_factory.register_builder(
        "p8b_actor_preserving", lambda **kwargs: players.PpoPlayerContinuous(**kwargs)
    )
